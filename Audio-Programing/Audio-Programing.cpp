...
public:
    SineOscillator(float freq, float amp) : frequency(freq), amplitude(amp) {
        offset = 2 * 3.1415 * frequency / sampleRate;
    }
    float process() {
...
int main() {
    ...
    for (int i = 0; i < sampleRate * duration; i++) {
        ...
        int intSample = static_cast<int> (sample * maxAmplitude);  // 32 -> 16 float
    ...
    writeToFile(audioFile, postAudioPosition - 8, 4);  //-8是因为 id和size不包括在内
